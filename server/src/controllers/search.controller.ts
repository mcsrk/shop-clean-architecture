...
export class SearchController {
	...
	public async fetchAllProducts(req: Request, res: Response) {
		try {
			...
			Logging.info(`[search.controller] Getting All products from ${companyPrefix} Ecommerce...`);

			/** 1. Fetch all products from Ecommerce
			 * Gets an array of type: [[mainProduct, ...variants],...,[mainProduct, ...variants]]
			 * format useful for traceability of the request
			 */

			const dbFormattedEcommerceProductsMatrix =
				(await this.ecommerceUseCase.fetchAllEcommerceProducts(companyPrefix)) ?? [];
			Logging.info(
				`[search.controller] Retrieved ${dbFormattedEcommerceProductsMatrix.length} products from ${companyPrefix} Ecommerce`,
			);
			...
			return res.status(200).json({
				...
				message: `OK`,
				result: {
					message: `Fetched all products into own database`,
					store: companyPrefix,
